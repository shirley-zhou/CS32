...
bool Multiset::insert(const ItemType& value)
{
	...
	if (m_distinct == DEFAULT_MAX_ITEMS)
		return false;
...
void Multiset::swap(Multiset& other)
{
	//first swap arrays
	int k = 0;
	for (; k < m_distinct && k < other.m_distinct; k++)
	{
		ItemType temp1 = m_data[k].m_item;
		int temp2 = m_data[k].m_instances;

		m_data[k].m_item = other.m_data[k].m_item;
		m_data[k].m_instances = other.m_data[k].m_instances;

		other.m_data[k].m_item = temp1;
		other.m_data[k].m_instances = temp2;
	}

	//if arrays being swapped are different sizes, copy over remaining elements
	for (; k < m_distinct; k++)
	{
		ItemType temp1 = m_data[k].m_item;
		int temp2 = m_data[k].m_instances;
	...
	//swap the other variables in Multiset
